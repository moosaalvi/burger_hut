...
if ($email != "moosa@gmail.com" && $password != "moosa") {
    $ad = "admin";
    echo "<script type='text/javascript'>alert('invalid admin's details!');</script>";

    header("location: login.php?admin=$ad");
    exit();
} ?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <style>
        body {
            ...
            margin: 0;
            padding: 0;
            background: url("img/gallery3.jpg") no-repeat;
...
<header style=>
        ...
        <nav>
            ...
            <li><a href="logout.php?admin_logout=$log" class="log">Logout</a></li>
    ...
    if (isset($_POST['btn'])) {
        ...
        if (move_uploaded_file($picfrom, $picto)) {
            if (!empty($pname) && !empty($pqty) && !empty($pprice) && !empty($pdtl) && !empty($picname)) {
                $query = "INSERT INTO fast_food(`p_name`, `p_qty`, `p_price`, `p_detail`, `p_img`) VALUES ('$pname', '$pqty', '$pprice', '$pdtl', '$picto')";
    ...
    <table>
        ...
        ?>
            <tr>
                ...
                <td class="actions">
                    <a href='update.php?id=<?php echo $data['product_id'] ?>' onclick="return confirmAction('Are you sure you want to update this item?')">Update</a>
                    <a href='delete.php?id=<?php echo $data['product_id'] ?>' onclick="return confirmAction('Are you sure you want to delete this item?')">Delete</a>
                </td>
            </tr>
        <?php
        }
